<div class="m-2 min-h-[calc(100vh-5rem)] overflow-hidden rounded-lg bg-white shadow-sm">
        ...
        @else
            <section class="rounded-md border border-green-300 bg-green-50 shadow-md">
                <div class="grid gap-5 p-5 xl:grid-cols-[1.2fr_1fr_auto] xl:items-center">
                    ...
                    <dl class="grid grid-cols-2 gap-3">
                        <div class="rounded-md border border-green-200 bg-white p-3"><dt class="text-xs font-semibold text-gray-500">Fondo inicial</dt><dd class="mt-1 text-lg font-black text-gray-900">{{ $currencySymbol }} {{ \App\Support\Decimal::format($session->opening_amount, 2) }}</dd></div>
                        <div class="rounded-md border border-green-200 bg-white p-3"><dt class="text-xs font-semibold text-gray-500">Efectivo esperado</dt><dd class="mt-1 text-lg font-black text-gray-900">{{ $currencySymbol }} {{ \App\Support\Decimal::format($breakdown['expected'], 2) }}</dd></div>
                    </dl>
            ...
            <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                @foreach([
                    ...
                    <article class="rounded-md border border-gray-300 bg-gray-100 p-4 shadow-md"><p class="text-xs font-semibold text-gray-600">{{ $label }}</p><p class="mt-2 text-xl font-black {{ $color }}">{{ $currencySymbol }} {{ \App\Support\Decimal::format($value, 2) }}</p></article>
                @endforeach
            </section>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($history as $item)
                            <tr><td class="px-4 py-3 font-bold">#{{ str_pad($item->id, 6, '0', STR_PAD_LEFT) }}<div class="mt-1 text-xs {{ $item->status === 'open' ? 'text-green-700' : 'text-gray-500' }}">{{ strtoupper($item->status) }}</div></td><td class="px-4 py-3"><strong>{{ $item->register->name }}</strong><div class="text-xs text-gray-500">{{ $item->user->name }}</div></td><td class="whitespace-nowrap px-4 py-3 text-xs text-gray-600">{{ $item->opened_at->timezone(config('cash.timezone'))->format('d/m/Y H:i') }}<div>{{ $item->closed_at?->timezone(config('cash.timezone'))?->format('d/m/Y H:i') ?: '—' }}</div></td><td class="px-4 py-3 text-right font-semibold">{{ $currencySymbol }} {{ \App\Support\Decimal::format($item->opening_amount, 2) }}</td><td class="px-4 py-3 text-right">{{ $item->expected_amount !== null ? $currencySymbol.' '.\App\Support\Decimal::format($item->expected_amount, 2) : '—' }}</td><td class="px-4 py-3 text-right">{{ $item->closing_amount !== null ? $currencySymbol.' '.\App\Support\Decimal::format($item->closing_amount, 2) : '—' }}</td><td class="px-4 py-3 text-right font-bold">{{ $item->difference !== null ? $currencySymbol.' '.\App\Support\Decimal::format($item->difference, 2) : '—' }}</td><td class="px-4 py-3 text-right"><button wire:click="viewSession({{ $item->id }})" class="rounded-md border border-gray-300 px-3 py-2 text-xs font-bold hover:bg-gray-100">Ver detalle</button></td></tr>
                        @empty
        ...
        <x-slot name="content">
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                @foreach([['BILLETES', $banknotes], ['MONEDAS', $coins]] as [$title, $group])
                    <section class="overflow-hidden rounded-md border border-gray-300"><h3 class="bg-gray-900 px-4 py-3 text-sm font-bold text-white">{{ $title }}</h3><div class="divide-y divide-gray-200">@foreach($group as $denomination)<label class="grid grid-cols-[1fr_90px_1fr] items-center gap-3 px-4 py-2"><span class="font-bold">{{ $currencySymbol }} {{ \App\Support\Decimal::format($denomination->value, 2) }}</span><input aria-label="Cantidad de {{ $currencySymbol }} {{ $denomination->value }}" inputmode="numeric" type="number" min="0" max="{{ config('cash.max_denomination_quantity') }}" step="1" wire:model.live.debounce.150ms="openingCounts.{{ $denomination->id }}" class="h-9 rounded-md border-gray-300 px-2 text-center text-sm"><span class="text-right font-semibold text-gray-700">{{ $currencySymbol }} {{ \App\Support\Decimal::format($this->lineSubtotal($denomination->id, 'opening'), 2) }}</span></label>@endforeach</div></section>
                @endforeach
            </div>
            @error('denominationCounts')<p class="mt-3 text-sm font-semibold text-red-600">{{ $message }}</p>@enderror
            <div class="mt-4 flex flex-col gap-3 rounded-md bg-gray-950 p-4 text-white sm:flex-row sm:items-center sm:justify-between"><div><p class="text-xs font-bold uppercase tracking-wider text-gray-400">Total contado</p><p class="mt-1 text-3xl font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($this->openingTotal, 2) }}</p></div><button type="button" wire:click="clearOpeningCount" class="rounded-md border border-white/30 px-4 py-2 text-sm font-bold">Limpiar conteo</button></div>
    ...
    <x-dialog-modal name="cash-movement" maxWidth="lg">
        ...
        <x-slot name="content">
            ...
            @if($session)<div class="mt-4 grid grid-cols-2 gap-3 rounded-md border border-gray-300 bg-gray-100 p-3"><div><p class="text-xs text-gray-500">Saldo actual</p><p class="font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($breakdown['expected'], 2) }}</p></div><div><p class="text-xs text-gray-500">Saldo después</p><p class="font-black">{{ $this->projectedBalance !== null ? $currencySymbol.' '.\App\Support\Decimal::format($this->projectedBalance, 2) : '—' }}</p></div></div>@endif
        </x-slot>
    ...
    <x-dialog-modal name="cash-closing" maxWidth="3xl">
        ...
        <x-slot name="content">
            @if(!$closingPreview)
                ...
                <div class="grid gap-4 lg:grid-cols-2">
                    @foreach([['BILLETES', $banknotes], ['MONEDAS', $coins]] as [$title, $group])
                        <section class="overflow-hidden rounded-md border border-gray-300"><h3 class="bg-gray-900 px-4 py-3 text-sm font-bold text-white">{{ $title }}</h3><div class="divide-y divide-gray-200">@foreach($group as $denomination)<label class="grid grid-cols-[1fr_90px_1fr] items-center gap-3 px-4 py-2"><span class="font-bold">{{ $currencySymbol }} {{ \App\Support\Decimal::format($denomination->value, 2) }}</span><input aria-label="Cantidad de cierre {{ $currencySymbol }} {{ $denomination->value }}" inputmode="numeric" type="number" min="0" max="{{ config('cash.max_denomination_quantity') }}" step="1" wire:model.live.debounce.150ms="closingCounts.{{ $denomination->id }}" class="h-9 rounded-md border-gray-300 px-2 text-center text-sm"><span class="text-right font-semibold">{{ $currencySymbol }} {{ \App\Support\Decimal::format($this->lineSubtotal($denomination->id, 'closing'), 2) }}</span></label>@endforeach</div></section>
                    @endforeach
                </div>
                @error('denominationCounts')<p class="mt-3 text-sm font-semibold text-red-600">{{ $message }}</p>@enderror
                <div class="mt-4 flex items-center justify-between rounded-md bg-gray-950 p-4 text-white"><div><p class="text-xs font-bold uppercase text-gray-400">Total contado</p><p class="text-3xl font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($this->closingTotal, 2) }}</p></div><button type="button" wire:click="clearClosingCount" class="rounded-md border border-white/30 px-4 py-2 text-sm font-bold">Limpiar</button></div>
            @else
                <div class="grid gap-3 sm:grid-cols-3"><div class="rounded-md border border-gray-300 p-4"><p class="text-xs font-semibold text-gray-500">Efectivo esperado</p><p class="mt-2 text-xl font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($closingPreview['expected'], 2) }}</p></div><div class="rounded-md border border-gray-300 p-4"><p class="text-xs font-semibold text-gray-500">Efectivo contado</p><p class="mt-2 text-xl font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($closingPreview['counted'], 2) }}</p></div><div class="rounded-md border-2 p-4 {{ $closingPreview['status'] === 'CUADRA' ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50' }}"><p class="text-xs font-bold">{{ $closingPreview['status'] }}</p><p class="mt-2 text-xl font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($closingPreview['difference'], 2) }}</p></div></div>
    ...
    <x-dialog-modal name="cash-closing-result" maxWidth="lg">
        ...
        <x-slot name="content">@if($closedSummary)<div class="text-center"><div class="text-sm font-bold {{ $closedSummary['status'] === 'CUADRA' ? 'text-green-700' : 'text-red-700' }}">{{ $closedSummary['status'] }}</div><p class="mt-3 text-3xl font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($closedSummary['difference'], 2) }}</p><div class="mt-5 grid grid-cols-2 gap-3 text-left"><div class="rounded-md bg-gray-100 p-3"><p class="text-xs text-gray-500">Esperado</p><p class="font-bold">{{ $currencySymbol }} {{ \App\Support\Decimal::format($closedSummary['expected'], 2) }}</p></div><div class="rounded-md bg-gray-100 p-3"><p class="text-xs text-gray-500">Contado</p><p class="font-bold">{{ $currencySymbol }} {{ \App\Support\Decimal::format($closedSummary['counted'], 2) }}</p></div></div></div>@endif</x-slot>
            ...
            @if($selectedHistorySession)
                ...
                <div class="mt-4 grid gap-4 lg:grid-cols-2">
                    @foreach([['CONTEO DE APERTURA', $selectedHistorySession->openingCount], ['CONTEO DE CIERRE', $selectedHistorySession->closingCount]] as [$title, $count])
                        <section class="rounded-md border border-gray-300"><h3 class="bg-gray-900 px-4 py-2 text-sm font-bold text-white">{{ $title }}</h3>@if($count)<div class="divide-y divide-gray-200">@foreach($count->lines as $line)@if($line->quantity > 0)<div class="flex justify-between px-4 py-2 text-sm"><span>{{ $currencySymbol }} {{ \App\Support\Decimal::format($line->denomination->value, 2) }} × {{ $line->quantity }}</span><strong>{{ $currencySymbol }} {{ \App\Support\Decimal::format($line->subtotal, 2) }}</strong></div>@endif @endforeach</div><div class="border-t border-gray-300 p-3 text-right font-black">Total {{ $currencySymbol }} {{ \App\Support\Decimal::format($count->total, 2) }}</div>@else<p class="p-4 text-sm text-gray-500">Sin conteo registrado (sesión histórica anterior a Fase 4.5).</p>@endif</section>
                    @endforeach
                </div>
                <section class="mt-4 overflow-hidden rounded-md border border-gray-300"><h3 class="bg-gray-900 px-4 py-2 text-sm font-bold text-white">MOVIMIENTOS</h3><div class="max-h-60 overflow-auto divide-y divide-gray-200">@forelse($selectedHistorySession->movements as $movement)<div class="flex justify-between gap-4 px-4 py-2 text-sm"><span>{{ strtoupper($movement->type) }} · {{ $movement->reason }}</span><strong>{{ $currencySymbol }} {{ \App\Support\Decimal::format($movement->amount, 2) }}</strong></div>@empty<p class="p-4 text-sm text-gray-500">Sin movimientos.</p>@endforelse</div></section>
                <div class="mt-4 grid gap-4 lg:grid-cols-2">
                    <section class="overflow-hidden rounded-md border border-gray-300"><h3 class="bg-gray-900 px-4 py-2 text-sm font-bold text-white">VENTAS DEL TURNO</h3><div class="max-h-48 overflow-auto divide-y divide-gray-200">@forelse($selectedHistorySession->sales as $sale)<div class="flex justify-between px-4 py-2 text-sm"><span>Venta #{{ $sale->id }} · {{ $sale->customer?->name }}</span><strong>{{ $currencySymbol }} {{ \App\Support\Decimal::format($sale->total, 2) }}</strong></div>@empty<p class="p-4 text-sm text-gray-500">Sin ventas vinculadas.</p>@endforelse</div></section>
                    <section class="overflow-hidden rounded-md border border-gray-300"><h3 class="bg-gray-900 px-4 py-2 text-sm font-bold text-white">REFUNDS DEL TURNO</h3><div class="max-h-48 overflow-auto divide-y divide-gray-200">@forelse($selectedHistorySession->refunds as $refund)<div class="flex justify-between px-4 py-2 text-sm"><span>Refund #{{ $refund->id }} · {{ $refund->paymentMethod?->name }}</span><strong>{{ $currencySymbol }} {{ \App\Support\Decimal::format($refund->amount, 2) }}</strong></div>@empty<p class="p-4 text-sm text-gray-500">Sin refunds vinculados.</p>@endforelse</div></section>
                </div>
                ...
                @if($selectedHistorySession->status === 'closed')<div class="mt-4 grid gap-3 sm:grid-cols-3"><div class="rounded-md border p-3"><p class="text-xs text-gray-500">Esperado</p><p class="font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($selectedHistorySession->expected_amount, 2) }}</p></div><div class="rounded-md border p-3"><p class="text-xs text-gray-500">Contado</p><p class="font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($selectedHistorySession->closing_amount, 2) }}</p></div><div class="rounded-md border p-3"><p class="text-xs text-gray-500">Diferencia</p><p class="font-black">{{ $currencySymbol }} {{ \App\Support\Decimal::format($selectedHistorySession->difference, 2) }}</p><p class="mt-1 text-xs text-gray-600">{{ $selectedHistorySession->closing_notes }}</p></div></div>@endif
            @endif
        </x-slot>
